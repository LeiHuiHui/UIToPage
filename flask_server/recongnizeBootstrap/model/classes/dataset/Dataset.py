...
class Dataset:
    ...
    def load(self, path, generate_binary_sequences=False):
        ...
        # one-hot encoding，保存在binary_vocabulary中
        self.voc.create_binary_representation()

        # 将 next_words one-hot encoding
    ...
    def append(self, sample_id, gui, img, to_show=False):
        # 对每对[img,gui]构建实际的训练时输入的样本。给个长度为len(gui)的GUI文本会产生len(gui)-CONTENT_LENGHT个训练样本
        if to_show:
            pic = img * 255
    ...
    def save_metadata(self, path):
        np.save("{}/meta_dataset".format(path), np.array([self.input_shape, self.output_size, self.voc.size]))
