...
class Vocabulary:
    def __init__(self):
        self.binary_vocabulary = {}  # one-hot encoding 的结果
        self.vocabulary = {}
        self.token_lookup = {}
        self.size = 0
    ...
    def save(self, path):
        # 写入voc中每个token的one-hot编码
        output_file_name = "{}/words.vocab".format(path)
        output_file = open(output_file_name, 'w')
        output_file.write(self.get_serialized_binary_representation())
        output_file.close()
        # 写入voc中每个token
        voc_file_name = "{}/vocabulary.vocab".format(path)
        voc_file = open(voc_file_name, 'w')
        voc_file.write(self.get_voacb_string())
        voc_file.close()
